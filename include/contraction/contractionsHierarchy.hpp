...
#ifndef INCLUDE_CONTRACTION_CONTRACTIONSHIERARCHY_HPP_
#define INCLUDE_CONTRACTION_CONTRACTIONSHIERARCHY_HPP_
#pragma once
...
public:
    ...
    */
        ...
        std::ostringstream &err)
    {
        ...
        if ( p_max > 0 ) {                    
            ...
            try {
                ...
                err << std::endl << "Boost exception during vertex contraction! " 
                    << dynamic_cast<std::exception const &>(except).what() << std::endl;
            }
            catch ( ... ) {
            ...
            for (const auto &w: out_vertices) {                
                ...
                if ( u != w ) {
                    ...
                    if (found_e && found_f && (!found_g || (found_g && (distances[w] < graph[g].cost)))) {
                        ...
                        {
                            if (!simulation) {
                                pgrouting::CH_edge ch_e = graph.process_shortcut(u, v, w);
                                log << "    Shortcut = (" << graph[u].id << ", " << graph[w].id << "), ";
    ...
    )
    {
        ...
        PQ minPQ;
        for (const auto &v: boost::make_iterator_range(boost::vertices(graph.graph))) {
            if (!(graph.get_forbidden_vertices()).has(v)) {
                minPQ.push(
                    std::make_pair(
                        process_vertex_contraction(graph, v, false, shortcuts, log, err), 
                        v
                    )
                );
            }
        ...
        while (!minPQ.empty()) {
            ...
            log << "  Vertex: " << graph[ordered_vertex.second].id 
                << ", min value of the queue: " << minPQ.top().first << std::endl
...
#endif  // INCLUDE_CONTRACTION_CONTRACTIONSHIERARCHY_HPP_
