...
public:
    ...
    double find_pmax(G &graph, V u, V v, Identifiers<V> out_vertices) {
        double p_max = 0.;
        E e, f;
    ...
    )
    {
        ...
        log << "Contraction of node " << v << std::endl;
        ...
        while (!adjacent_in_vertices.empty()) {
            ...
            try {
                ...
                double p_max = find_pmax(graph, u, v, adjacent_out_vertices);
                log << "Found pmax = " << p_max << " for node " << v << std::endl;
                ...
                log << "First labelling done for node " << v << std::endl;
                ...
                error << "Vertex contraction " << except.what() << std::endl;
            }
        }
        return shortcuts - old_edges;
    }

    void doContraction(G &graph, std::ostringstream &log) {
        ...
        BGL_FORALL_VERTICES_T(v, graph.graph, B_G) {
            if (!forbiddenVertices.has(v)) {
                log << "Vertex " << v << std::endl;
                minPQ.push(std::make_pair(process_vertex_contraction(graph, v, log), v));
            }
        }
