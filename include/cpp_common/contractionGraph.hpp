...
class Pgr_contractionGraph
 ...
 public:
  ...
  using PQ =
      typename std::priority_queue<V_p, std::vector<V_p>, std::greater<V_p> >;
  ...
  */
  int64_t get_next_id() { return --min_edge_id; }
  ...
  */
  void setForbiddenVertices(Identifiers<V> m_forbidden_vertices) {
  ...
  */
  Identifiers<V> getForbiddenVertices() { return forbiddenVertices; }
  ...
  */
  ...
  void set_vertices_order(std::vector<std::pair<int64_t, int64_t> > vertices) {
    for (const auto &v : vertices) {
      this->graph[this->vertices_map[v.first]].set_order(v.second);
    }
  }
  /*!
      @brief defines the metric and hierarchy at the level of the nodes, from a
     given priority queue
      @param [in] PQ priority_queue
      @return void
  */
  void set_vertices_metric_and_hierarchy(PQ priority_queue,
    ...
    while (!priority_queue.empty()) {
      ...
      log << "(" << ordered_vertex.first << ", "
          << (this->graph[ordered_vertex.second]).id << ")" << std::endl;
      log << " metric = " << (this->graph[ordered_vertex.second]).metric
          << " order = " << (this->graph[ordered_vertex.second]).vertex_order
          << std::endl;
    }
  }
  ...
  /*!
      ...
      @param [in] v vertex_descriptor
      @return Identifiers<V>: The set of vertex descriptors adjacent to the
     given vertex *v*
  */
  ...
  /*!
      @brief get the vertex descriptors of adjacent vertices of *v*
      @param [in] v vertex_descriptor
      @return Identifiers<V>: The set of out vertex descriptors adjacent to the
     given vertex *v*
  */
  ...
  /*!
      @brief get the vertex descriptors of adjacent vertices of *v*
      @param [in] v vertex_descriptor
      @return Identifiers<V>: The set of in vertex descriptors adjacent to the
     given vertex *v*
  */
  ...
  */
  std::vector<E> get_shortcuts() {
    ...
    std::sort(eids.begin(), eids.end(), [&](E lhs, E rhs) {
  ...
  */
  ...
  friend std::ostream &operator<<(std::ostream &os,
    ...
    EO_i out, out_end;
    for (auto vi = vertices(g.graph).first; vi != vertices(g.graph).second;
         ++vi) {
      if ((*vi) >= g.num_vertices()) break;
      os << g.graph[*vi].id << "(" << (*vi) << ")"
         << g.graph[*vi].contracted_vertices() << std::endl;
      os << " out_edges_of(" << g.graph[*vi].id << "):";
      for (boost::tie(out, out_end) = out_edges(*vi, g.graph); out != out_end;
           ++out) {
        os << ' ' << g.graph[*out].id << "=(" << g.graph[g.source(*out)].id
           << ", " << g.graph[g.target(*out)].id << ") = " << g.graph[*out].cost
           << "\t";
      }
      os << std::endl;
    }
  ...
  CH_edge process_shortcut(V u, V v, V w) {
    ...
    // Create shortcut
    CH_edge shortcut(get_next_id(), (this->graph[u]).id, (this->graph[w]).id,
                     cost);
  ...
  void process_ch_shortcut(V u, V v, V w, std::vector<E> &shortcuts,
                           std::ostringstream &log) {
    bool found_e;
    E e;
