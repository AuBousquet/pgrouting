...
static void process(char *edges_sql, char *combinations_sql, ArrayType *starts,
                    ArrayType *ends,

                    bool directed, bool only_cost, Path_rt **result_tuples,
                    size_t *result_count) {
  pgr_SPI_connect();
  char *log_msg = NULL;
  ...
  pgr_hbd_dijkstra(edges_sql, combinations_sql, starts, ends,
...
PGDLLEXPORT Datum _pgr_hbd_dijkstra(PG_FUNCTION_ARGS) {
  ...
  if (funcctx->call_cntr < funcctx->max_calls) {
    ...
    result_tuples[call_cntr].start_id =
        result_tuples[call_cntr].edge < 0 ? 1 : seq + 1;
